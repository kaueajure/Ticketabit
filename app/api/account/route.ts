...
import { changeOwnPassword, updateOwnAccount, updateOwnTheme } from "@/lib/account";
...
export async function PATCH(request: Request) {
  try {
    ...
    const body = await request.json() as { name?: string; theme?: ThemePreference; currentPassword?: string; newPassword?: string };
    if (typeof body.name === "string") {
      await updateOwnAccount(user.id, body.name);
      return NextResponse.json({ ok: true });
    }
    if (typeof body.theme === "string") {
      await updateOwnTheme(user.id, body.theme);
